<?php
    ...
    class CheckoutWizard extends ZervWizard
    {
        function CheckoutWizard()
        {
            ...
            {
            	$this->addStep('Fieldtype', _PED_FIELD_TYP_SEL);
            	if ($this->getValue('fieldtype') == 'selectbox' || $this->getValue('fieldtype') == 'radiobutton')
	            {
	            	$this->addStep('lookup', _PED_FIELD_ADD_VALUE);
	        	}
        	}
            
        	$this->addStep('Settings', _PED_FIELD_PARAM);
        	if ($this->getValue('hassearch') == 'hassearch')
        	{
	        	$this->addStep('search', _PED_SEARCH_PARAMFIELD);
        	}
        	if ($this->getValue('fieldtype') != 'picture')
        	{
	        	$this->addStep('defaultvalue', _PED_FIELD_DEFAUT);
        	}
        	$this->addStep('confirm', _PED_FIELDCONFIRM);
        }
        ...
        function process_Fieldname(&$form)
        {
            ...
            if (strlen($name) > 0)
                ...
                $this->addError('name', _PED_FIELD_NAM);
                
            $fieldexplenation = $this->coalesce($form['explain']);
			if (strlen($fieldexplenation) > 0)
                $this->setValue('explain', $fieldexplenation);
            else
                $this->addError('explain', _PED_FIELD_EXPLAN1);
        ...
        function prepare_Fieldtype()
        {
			...
			$this->fieldtype[] = array( 'value' => "selectbox", 'description' => _PED_DROPDOWNFIELD);
			$this->fieldtype[] = array( 'value' => "textbox", 'description' => _PED_TEXTBOXFIELD);
			$this->fieldtype[] = array( 'value' => "textarea", 'description' => _PED_TEXTAREAFIELD);
			$this->fieldtype[] = array( 'value' => "dateselect", 'description' => _PED_DATEFIELD);
			$this->fieldtype[] = array( 'value' => "urlfield", 'description' => _PED_URLFIELD);
        }
        ...
        function prepare_Settings()
        {
			...
			if (!$this->getValue('field') == 0) // field allready exists (editing mode)
			{
	        	...
	        	{
		        	$hs = $row['HasSearch'];
		        	if ($hs == "1") { $this->setValue('hassearch', "hassearch"); }
		        	$vip = $row['ViewInPedigree'];
		        	if ($vip == "1") { $this->setValue('viewinpedigree', "viewinpedigree"); }
		        	$via = $row['ViewInAdvanced'];
		        	if ($via == "1") { $this->setValue('viewinadvanced', "viewinadvanced"); }
		        	$vipie = $row['ViewInPie'];
		        	if ($vipie == "1") { $this->setValue('viewinpie', "viewinpie"); }
		        	$vil = $row['ViewInList'];
		        	if ($vil == "1") { $this->setValue('viewinlist', "viewinlist"); }
		        	$lit = $row['Litter'];
		        	if ($lit == "1") { $this->setValue('Litter', "Litter"); }
		        	$Glit = $row['Generallitter'];
		        	if ($Glit == "1") { $this->setValue('Generallitter', "Generallitter"); }
	        	}	
			}
        }
        ...
        function prepare_defaultvalue()
        {
			...
			if (!$this->getValue('field') == 0) // field allready exists (editing mode)
			{
	        	...
	        	while ($row = $xoopsDB->fetchArray($result)) 
	        	{
		        	...
		        	{
			        	$sql = "SELECT * from ".$xoopsDB->prefix("stamboom_lookup".$this->getValue('field'))." order by 'order'";
			        	$fc = 0;
			        	$result = $xoopsDB->query($sql);
	        			while ($row = $xoopsDB->fetchArray($result)) 
	        			{
        ...
        function completeCallback()
        {
		    ...
		    if ($viewinpedigree == "viewinpedigree") { $viewinpedigree = "1"; }
		    else { $viewinpedigree = "0"; }
		    //show in advanced
		    $viewinadvanced = $this->getValue('viewinadvanced');
		    if ($viewinadvanced == "viewinadvanced") { $viewinadvanced = "1"; }
		    else { $viewinadvanced = "0"; }
		    //show in pie
		    $viewinpie = $this->getValue('viewinpie');
		    if ($viewinpie == "viewinpie") { $viewinpie = "1"; }
		    else { $viewinpie = "0"; }
		    //view in list
		    $viewinlist = $this->getValue('viewinlist');
		    if ($viewinlist == "viewinlist") { $viewinlist = "1"; }
		    else { $viewinlist = "0"; }
		    //add a litter
		    $Litter = $this->getValue('Litter');
		    if ($Litter == "Litter") { $Litter = "1"; }
		    else { $Litter = "0"; }
		    //general litter
		    $Generallitter = $this->getValue('Generallitter');
		    if ($Generallitter == "Generallitter") { $Generallitter = "1"; }
		    else { $Generallitter = "0"; }
			...
			else //this is a new field
			{
		        ...
		        else 
		        {
			         ...
			         $createtable = "CREATE TABLE ".$xoopsDB->prefix("stamboom_lookup".$nextfieldnum)." (`ID` INT( 10 ) NOT NULL ,`value` VARCHAR( 255 ) NOT NULL, `order` INT( 10 )) type = MyISAM";
					 ...
					 {
						$y = $x -1;
			         	$sql ="INSERT INTO ".$xoopsDB->prefix("stamboom_lookup".$nextfieldnum)." ( `ID` , `value`, `order`) VALUES ('".$y."', '".$this->getValue('lookup'.$x)."','".$y."')";
			         	mysql_query($sql);
		         	 }
