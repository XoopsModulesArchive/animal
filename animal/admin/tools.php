...
{
	global $xoopsTpl;
	$form = "Please use the menu items on the left to run preset database queries or request your own queries from <a href =\"http://www.animalpedigree.com/support\">AnimalPedigree.com</a>";	
	$xoopsTpl->assign("form", $form);
}
...
function settings()
{
	...
	$form->addElement(new XoopsFormLabel(_PED_EXPLAIN, 'Use this field to set if you would like to use the owner/breeder fields of the database.<br />As the name suggests the owner/breeder fields let you record and display information about the owner and or breeder.<br />The owner/breeder menu items will also be affected by this setting.'));
...
function lang()
{
	...
	$form->addElement(new XoopsFormLabel(_PED_EXPLAIN, 'Use this field to set the animal type which will be used in the application.<br />This field is the plural of the previous field<br /><i>example : </i>snakes, pigeons, dogs, owls<br /><br />The value should fit in the sentence below.<br />No <b>'.$moduleConfig['animalTypes'].'</b> meeting your query have been found.'));
	...
	$form->addElement(new XoopsFormLabel(_PED_EXPLAIN, 'Use this field to set the text you would like to display for the welcome page.<br /><br />You may use the follwing variables :<br />[animalType] = '.$moduleConfig['animalType'].'<br />[animalTypes] ='.$moduleConfig['animalTypes'].'<br />[numanimals] = number of animals in the database.'));
	$form->addElement(new XoopsFormButton('', 'button_id', 'Submit', 'submit'));	
	$xoopsTpl->assign("form", $form->render());
}
