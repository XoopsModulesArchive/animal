...
$pages .= "-&nbsp;";
$pages .= "<a href=\"seldog.php?gend=".$gend."&curval=".$curval."&letter=Å\">Å</a>&nbsp;";
...
if ($gend == '0')
{
	$dogs [] = array ('id' => "0", 'name' => "", 'gender' => "", 'link' => "<a href=\"updatepage.php?gend=".$gend."&curval=".$curval."&thisid=0\">".strtr(_PED_ADD_SIREUNKNOWN, array( '[father]' => $moduleConfig['father'] ))."</a>", 'colour' => "", 'number' => "", 'usercolumns' => $empty);
}
else
{
	$dogs [] = array ('id' => "0", 'name' => "", 'gender' => "", 'link' => "<a href=\"updatepage.php?gend=".$gend."&curval=".$curval."&thisid=0\">".strtr(_PED_ADD_DAMUNKNOWN, array( '[mother]' => $moduleConfig['mother'] ))."</a>", 'colour' => "", 'number' => "", 'usercolumns' => $empty);
}
...
while ($row = $xoopsDB->fetchArray($result)) 
{
	...
	if ($gend == '0')
	{
		$dogs[] = array ('id' => $row['ID'], 'name' => $name, 'gender' => '<img src="images/male.gif">', 'link' => "<a href=\"updatepage.php?gend=".$gend."&curval=".$curval."&thisid=".$row['ID']."\">".$name."</a>",'colour' => "", 'number' => "", 'usercolumns' => $columnvalue);	
	}
	else
	{
		$dogs[] = array ('id' => $row['ID'], 'name' => $name, 'gender' => '<img src="images/female.gif">', 'link' => "<a href=\"updatepage.php?gend=".$gend."&curval=".$curval."&thisid=".$row['ID']."\">".$name."</a>",'colour' => "", 'number' => "", 'usercolumns' => $columnvalue);
	}	
}
...
//add data to smarty template
if ($gend == '0') { $seltitparent = strtr(_PED_FLD_FATH, array( '[father]' => $moduleConfig['father'] )); } 
else { $seltitparent = strtr(_PED_FLD_MOTH, array( '[mother]' => $moduleConfig['mother'] )); }
