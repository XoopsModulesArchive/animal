...
if (!isset($f)) { virt(); }
if (isset($f) && $f == "dam") { dam(); }
if (isset($f) && $f == "check") { check(); }
...
function virt()
{
	...
	$pages .="-&nbsp;";
	$pages .= "<a href=\"virtual.php?r=1&st=0&l=Å\">Å</a>&nbsp;";
...
function dam()
{
	...
	$pages .= "-&nbsp;";
	$pages .= "<a href=\"virtual.php?f=dam&selsire=".$selsire."&st=0&l=Å\">Å</a>&nbsp;";
	...
	while ($row = $xoopsDB->fetchArray($result)) 
	{
		...
		$dogs[] = array ('id' => $row['d_id'], 'name' => $name, 'gender' => '<img src="images/female.gif">', 'link' => "<a href=\"virtual.php?f=check&selsire=".$selsire."&seldam=".$row['d_id']."\">".$name."</a>",'colour' => "", 'number' => "", 'usercolumns' => $columnvalue);		
	}
...
function check()
{
	...
	if (isset($_GET['selsire'])) { $selsire = $_GET['selsire']; }	
	if (isset($_GET['seldam'])) { $seldam = $_GET['seldam']; }
	...
	$xoopsTpl->assign("form", "<a href=\"coi.php?s=".$selsire."&d=".$seldam."&dogid=&detail=1\">"._PED_VIRTUALBUT."</a>");
}
