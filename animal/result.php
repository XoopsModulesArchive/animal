...
{
	redirect_header("index.php",30,_PED_SEARCH_SHORT );
}
...
if ($_GET['l'] == "1" || $_GET['l'] == "LIKE") { $l = "LIKE"; }
else { $l = "="; }

if (!$_GET['o']) { $o = "NAAM"; }
else { $o = $_GET['o']; }

if (!isset($_GET['d'])) { $d = "ASC"; }
else { $d = $_GET['d']; }
...
if ($numresults == 0)
{
	...
	//echo $numdog;
	redirect_header("index.php",300,strtr(_PED_SEARCH_NO, array( '[animalTypes]' => $moduleConfig['animalTypes'] )));
}
...
if ($numpages > 1)
{
	if ($cpage > 1)	
	{
		$pages .= "<a href=\"result.php?f=".$f."&amp;l=".$l."&amp;w=".$whe."&amp;o=".$o."&amp;d=".$d."&amp;st=".($st-$perp)."\">"._PED_PREVIOUS."</a>&nbsp;&nbsp;";
	}
}
//create numbers
...
{
	if ($cpage < ($numpages))	
	{ $pages .= "<a href=\"result.php?f=".$f."&amp;l=".$l."&amp;w=".$whe."&amp;o=".$o."&amp;d=".$d."&amp;st=".($st+$perp)."\">"._PED_NEXT."</a>&nbsp;&nbsp"; }
}
...
while ($row = $xoopsDB->fetchArray($result)) 
{
	...
	$dogs[] = array ('id' => $row['ID'], 'name' => $name, 'gender' => $gender, 'link' => "<a href=\"pedigree.php?pedid=".$row['ID']."\">".$name."</a>",'colour' => "", 'number' => "", 'usercolumns' => $columnvalue);
}
