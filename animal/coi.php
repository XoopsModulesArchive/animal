...
if (! isset ($verbose )) $verbose  = 0   ;   // don't display different steps of ICs calculation
if (! isset ($detail  )) $detail   = 1   ;   // don't display detail results [faster]
if (! isset ($nb_maxi )) $nb_maxi  = 600 ;   // maximum count of distinct ascendants
if (! isset ($nb_gen  )) $nb_gen   = 8   ;   // maximum count of generations of ascendants
if (! isset ($pedigree)) $pedigree = 0   ;   // dont't display sketch pedigree [faster]
if (! isset ($max_dist))                     // maximum length of implex loops
{
	if ($nb_gen  > 9) $max_dist = 14 ;
	else if ($nb_gen == 9) $max_dist = 17 ;
	else if ($nb_gen == 8) $max_dist = 18 ;
	else                   $max_dist = 99 ;
}
...
function chrono_sort ()
 { global $IDs, $inds, $fathers, $mothers, $chrono, $nl, $detail ;    $impr = 0 ;
   $modif = 1 ; $nloop = 0 ; $nba = count ($IDs) ;
   // print_r ($IDs) ;
   // echo "<b>231 : $IDs[231] $fathers[231] $mothers[231] $chrono[231] $inds[231] </b><br />\n" ;
   foreach ($IDs as $i=>$v) $chrono[$i] = 1 ;  // initialize all chronological ranks to 1
   $chrono[0] = 0 ;                            // except animal #0 (at 0 rank).
   while ($modif && $nloop < 40)
     { $modif = 0 ; $nloop ++ ;
       for ($i=1 ; $i < $nba ; $i++)
         { $s = $fathers[$i] ; if ($s) $s = $inds[$s] ;
           $d = $mothers[$i] ; if ($d) $d = $inds[$d] ;
           if ($s && $chrono[$s] <= $chrono[$i])
             { $chrono[$s] = $chrono[$i] +1 ; $modif = 1 ; }
           if ($d && $chrono[$d] <= $chrono[$i])
             { $chrono[$d] = $chrono[$i] +1 ; $modif = 1 ; } } }
   if ($nloop == 40) die ("Endless loop detected. Stopped.") ;
   ...
   if ($impr)
     { echo "</center><pre>$nl $nl" ;
       foreach ($chrono as $i=>$val)  echo "<b>$i</b> : $val $IDs[$i] $fathers[$i] $mothers[$i] $nl" ;
       echo "</pre>$nl" ; die ("</html>") ; }
   $inds = array_flip ($IDs) ;
   return 0 ; }
...
function fetch_record ($s)
  { global $database ;
    ...
    $n = 0 ; if ($r) $n = mysql_num_rows ($r) ;
    if ($n == 0) $record = array ('0') ;
    else $record = mysql_fetch_array ($r) ;
    return $record ; }

function count_all ($ind, $gen)
  { global $inds, $nb_gen, $nb_all, $fathers, $mothers ;
    if ($ind) $nb_all ++ ;
    $s = $fathers[$ind] ; $d = $mothers[$ind] ;
    if ($s && $gen < $nb_gen) count_all ($s, $gen+1) ;
    if ($d && $gen < $nb_gen) count_all ($d, $gen+1) ;    
    return 0 ; }
  ...
  { global $implx, $couls, $nl ;
    ...
    foreach ($implx as $im=>$impl)
      { if ($impl[0] == $ch or $impl[1] == $ch)
          { if ($niv > 1 && $first) echo "<br />$nl" ; else echo "&nbsp;&nbsp;&nbsp;" ;
            $i = $im + 1 ; $j = min ($im, 6) ; $c = $couls[$j] ; $first = 0 ;
            echo '<font color=' . $c . ' size="+2"><b>*' . $i . '*</b></font>' ; } }
    return 0 ; }

function output_animal ($ind, $gen, $class)
  { global $depth, $IDs, $fathers, $mothers, $nl ;
    if ($gen > $depth) return 0 ;
    $cell_content = "&Oslash;" ;
    if ($ind or $gen == 0)
      { $ID = $IDs[$ind] ;
        $ani = set_name ($ID) ; $name = $ani[1] ; 
        $name = $ID ;
    ...
    if ($gen < $depth)
      { 
	      	$sire = 0 ; if ($ind or $gen == 0) $sire = $fathers[$ind] ; output_animal ($sire, $gen +1, "0") ; 
        	$dam  = 0 ; if ($ind or $gen == 0) $dam  = $mothers[$ind] ; output_animal ($dam , $gen +1, "1") ; 
      }
    else echo "</tr><tr>$nl" ;
    return 0 ; }
...
function SKETCH_PEDIGREE ()
  { global $nl, $detail, $depth, $IDs ;
    // print_r ($IDs) ;
    echo $nl . "<br />" . $nl .
      '<table border="3" cellpadding="4" width="85%"" cellpadding="0" cellspacing="0">' . $nl .
      '<tr><th colspan="10" align="center">SKETCH &nbsp; PEDIGREE &nbsp; OF COMMON PROGENY</th></tr>' . $nl .
      '<tr align="center" valign="middle"><th>Progeny</th><th>' . _("Sire / Dam") . '</th>' ;
    if ($depth >= 2) echo '<th>' . _("Grandparents") . '</th>' . $nl ;
    if ($depth >= 3) echo '<th>' . _("Great-Grandparents") . '</th>' . $nl ;
    if ($depth >= 4) echo '<th>3xGr. P.</th>' . $nl ;
    if ($depth >= 5) echo '<th>4xGr. P.</th>' . $nl ;
    if ($depth >= 6) echo '<th>5xGr. P.</th>' . $nl ;
    if ($depth >= 7) echo '<th>6xGr. P.</th>' . $nl ;
    echo '</tr><tr>' ;
...
function GENEALOGY ()
  { global $IDs, $fathers, $mothers, $inds, $nb_gen, $nb_maxi, $nbani, $nl, $sql1 ;   $impr = 0 ;
    $fathers[0] = $IDs[1] ; $mothers[0] = $IDs[2] ;
    $fathers[1] = 0 ; $mothers[1] = 0 ; $fathers[2] = 0 ; $mothers[2] = 0 ; $last = 2 ;
    if ($impr) echo "<!-- genealogy 'de cujus' (gener. 0) : $IDs[0] = $IDs[1] x $IDs[2] -->$nl" ;
    ...
    for ($nloop=1, $tot=2 ; $last <= $nb_maxi && $nloop <= $nb_gen ; $nloop++)
      { $nbtot += $tot ;    // count of total known ascendants within $nb_gen generations
        ...
        $generation = array () ;
        $tot = 0 ;
        if ($impr) echo "    [$list]$nl" ;

        // HERE IS FETCHED EACH TRIPLET [ID, sire_ID, dam_ID] :
        $r = mysql_query ("$sql1 IN ($list)") ;
        while ($rec = mysql_fetch_array ($r))
            { $a = $rec[0] +0 ; $s = $rec[1] +0 ; $d = $rec[2] +0 ;
              if (! $a) echo "ERROR : $a = $s x $d for list = '$list'<br />\n" ;
              if ($s) $tot ++ ; if ($d) $tot ++ ;
              $j = array_keys($IDs, $a) ; $j=$j[0] ; $fathers[$j] = $s ; $mothers[$j] = $d ;
              if ($s && ! in_array ($s, $IDs)) { $i = ++ $last ; $IDs[$i] = $s ;
                   $fathers[$i] = 0 ; $mothers[$i] = 0 ; if ($s) $generation[] = $s ; }
              if ($d && ! in_array ($d, $IDs)) { $i = ++ $last ; $IDs[$i] = $d ;
                   $fathers[$i] = 0 ; $mothers[$i] = 0 ; if ($s) $generation[] = $d ; }
              if ($impr) echo "<pre>genealogy ascendant (gener. $nloop) : $a = $s x $d  [tot = $tot]$nl</pre>" ; }
        if (! count ($generation)) break ; }
         
    if ($nloop <= $nb_gen) $nb_gen = $nloop ;  // tree cut by $nb_maxi !
         
    reset ($IDs) ; $inds = array_flip ($IDs) ;
...
function dist_p ($p)
  { global $IDs, $fathers, $mothers, $pater, $nb_gen, $detail, $nl ;  
    // Anim #P is the sire
    $listall = array ($p) ; $listnew = array ($p) ; $pater = array () ; $pater[$p] = 1 ;
    for ($nloop=2 ; $nloop < ($nb_gen +1) ; $nloop++)
      { $liste = array () ; 
        foreach ($listnew as $i)
            { $s = $fathers[$i] ; $d = $mothers[$i] ;
              if ($s && ! $pater[$s]) $pater[$s] = $nloop ; // least distance from $s to sire's progeny
              if ($d && ! $pater[$d]) $pater[$d] = $nloop ; // least distance from $d to sire's progeny
              if ($s) $liste[] = $s ; if ($d) $liste[] = $d ; }
        if (! count ($liste)) break ;
...
function dist_m ($m)
  { global $IDs, $fathers, $mothers, $mater, $nb_gen, $detail, $nl ; 
    // Anim #M is the dam
    $listall = array ($m) ; $listnew = array ($m) ; $mater = array () ; $mater[$m] = 1 ;
    for ($nloop=2 ; $nloop <= ($nb_gen +1) ; $nloop++)
      { $liste = array () ; 
        foreach ($listnew as $i)
            { $s = $fathers[$i] ; $d = $mothers[$i] ;
              if ($s && ! $mater[$s]) $mater[$s] = $nloop ; // least distance from $s to dam's progeny
              if ($d && ! $mater[$d]) $mater[$d] = $nloop ; // least distance from $d to dam's progeny
...
function calc_dist ()       /* Common Ascendants and their distances */
  { global $IDs, $fathers, $mothers, $nbanims, $pater, $mater, $empty, $nb_gen, $nl ;
    ...
    dist_p ($fathers[0]) ;  $dmax = 0 ;         $impr = 0 ;
    $dmx = 7 ; if ($detail) $dmx += 2 ;
    ...
    foreach ($pater as $i=>$p)
      { if ($p)
          { $m = $mater[$i] ;
            if ($m)
              { $di = $p + $m ;
                if ($impr) echo " $i : $p + $m = $di <br />$nl" ;
                if (! $dmax) $dmax = $dmx + $di - ceil ($di / 2.) ;
                if ($di > ($dmax +2)) continue ;
                $distan[$i] = $di ;
      }   }   }
    if (! $dmax) $dmax = 2 * $nb_gen - 2 ;
    return $distan ; }
...
function pater_side ($p, $m, $a, $pdist)
    { global $mater, $fathers, $mothers, $marked, $chrono, $paternal_rank ;
      ...
      if ($mater[$p] or $a)  mater_side ($p, $m, $a, $pdist) ;
...
function CONSANG ($a)
    { global $fathers, $mothers, $ICknown, $COIs, $nl ;
      if (!$a or $ICknown[$a]) return 0 ;
      if ($a == -1) $a = 0 ;  // particular case : a= -1 means Anim #0 (to bypass above test)
      ...
      $p = $fathers[$a] ; $m = $mothers[$a] ;
      if (!$p or !$m) { $COIs[$a] = $IC_if_deadend ; $ICknown[$a] = 2 ; return 0 ; }
      
      if ($verbose) echo "</center><pre>$nl" ;
      pater_side ($p, $m, $a, 1) ;  // launch tree exploration
      if ($verbose) echo "</pre><center>$nl" ;
      
      $ICknown[$a] = 1 ;
      $p = $fathers[$a] ; $m = $mothers[$a] ;
      foreach ($fathers as $i=>$pere)  /* siblings share the same COI value */
        if ($i <> $a && $pere == $p && $mothers[$i] == $m)
          { $COIs[$i] = $COIs[$a] ; $ICknown[$i] = 1 ; }
...
function boucle ($nb_gen, $nloop)
  { global $fathers, $mothers, $nbanims, $listing, $nl ; 
    $nbtot = 0 ; $listing = "" ;
    if ($nloop < ($nb_gen +20)) $nloop = $nb_gen +20 ;
    $list = array (0=>1) ;     /* initialize list with Anim0 (rank = 1) */
    for ($j=1 ; $j < $nloop ; $j++)
       { $new = 0 ;
         foreach ($list as $i=>$rank)
           { if (($s = $fathers[$i]))
               { if (! $list[$s]) { $new = 1 ; if ($j < $nb_gen) $nbtot += 1 ; }
                 $list[$s] = $rank +1 ;
                 if ($j < $nb_gen) $nbtot += 1 ;
                 if ($j > $nloop -10) $listing .= "Loop $j: Animal #$s " . $IDs[$s] . $nl ; }
             if (($d = $mothers[$i]))
               { if (! $list[$d]) { $new = 1 ; if ($j < $nb_gen) $nbtot += 1 ; }
                 $list[$d] = $rank +1 ;
                 if ($j < $nb_gen) $nbtot += 1 ;
                 if ($j > $nloop -10) $listing .= "Loop $j: Animal #$d " . $IDs[$d] . $nl ; } }
         if (! $new) break ;
       }
    if ($new) $nbtot = 0 ;  /* Endless loop detected (see listing) */
    return $nbtot ; }
           ...
           { $name = html_accents ($name) ;
             ...
             if ($affx)
               { $affix = fetch_record ("$sql2bis '$affx'") ;
                 ...
                 if ($type[0] == 'P') $name = '<i>'.$affixe."</i>&nbsp;".$name ;
                 if ($type[0] == 'S') $name = $name."&nbsp;<i>".$affixe.'</i>' ; }
             $ani[1] = $name ; } }
    return $ani ; }

function Ems_ ($ems)
  { if (function_exists ("Ems")) return Ems ($ems) ;
    if (!$ems) return "&nbsp;" ; $e = str_replace (" ", "+", $ems) ;
    $res = '<a href="#" style="text-decoration:none;" onClick="' .
      "window.open('http://www.somali.asso.fr/eros/decode_ems.php?$e'," .
      "'', 'resizable=no,width=570,height=370')" . '"' . "><b>$ems</b></a>" ;
    return $res ; }
...
function one_animal ($ID)  {
    ...
    $nb_children = $nb[0] ;
    if ($nb_children == 0) $nb_children = _PED_COI_NO;
    //$dogid = $animal[0];
    $content.= "<tr><td><a href=\"dog.php?id=".$ID."\">".stripslashes($name)."</a>" ;
    // if ($nb_enf == 0) echo ' &oslash;' ; 
    if ($val) $content .= $val ;
    if ($sex == 1) { $geslacht = "<img src=\"images/female.gif\">"; }
    if ($sex == 0) { $geslacht = "<img src=\"images/male.gif\">"; }
...
$a += 0 ; $s += 0 ; $d += 0 ;            // [IDs are numbers]

$xoopsTpl->assign("ptitle", _PED_COI_CKRI);
$xoopsTpl->assign("pcontent", strtr(_PED_COI_CKRI_CT, array('[animalType]' => $moduleConfig['animalType'])));
...
$maxn = $maxn_ ; $maxr = $maxr_ ;
$cinnamon = 0 ; $chocolat = 0 ; $dilution = 0 ; $sexlred = 0 ;
...
if (! isset ($nb_gen)) { $nb_gen = 7 ; if ($detail) $nb_gen = 9 ; }
else if ($nb_gen < $pedigree) $nb_gen = $pedigree ;

$IDs = array ($de_cujus +0, $codec1 +0, $codec2 +0) ;  /* Structuring animal IDs into memory */

$nbanims = GENEALOGY () ;   // ************************************************************* //

for ($i=0 ; $i <= $nbanims ; $i++) $empty[$i] = 0 ;

foreach ($fathers as $i=>$a) if ($a) $fathers[$i] = $inds[$a] ;  /* Replace parents codes */
foreach ($mothers as $i=>$a) if ($a) $mothers[$i] = $inds[$a] ;  /*   by  their  indices  */
...
$f = $empty ; $ICknown = $empty ; $deltaf = $empty ; $marked = $empty ;
...
$nf = ceil (100 * $COIs[0]) ;
     if ($nf >= 55) $w = _PED_COI_HUGE;
else if ($nf >= 35) $w = _PED_COI_VHIG;
else if ($nf >= 20) $w = _PED_COI_HIGH;
else if ($nf >= 10) $w = _PED_COI_MEDI;
else if ($nf >= 05) $w = _PED_COI_LOW;
else if ($nf >= 02) $w = _PED_COI_VLOW;
else if ($nf >= 01) $w = _PED_COI_VVLO;
else                $w = _PED_COI_TLTB;
$w = _PED_COI_TVI." ".$w;

$nb_all = 0 ; count_all (0, 0) ;  // count all ascendants in flat tree
...
$xoopsTpl->assign("TCAexplain", strtr(_PED_COI_TCAEX, array( '[animalType]' => $moduleConfig['animalType'], '[animalTypes]' => $moduleConfig['animalTypes'], '[children]' => $moduleConfig['children'],  '[mother]' => $moduleConfig['mother'],  '[father]' => $moduleConfig['father'])));

if ($detail)
  { if ($verbose) $verbose = 0 ;
    if (count ($COIs) > 1)
      { 
        $ICs = $COIs ; arsort ($ICs) ; $j=1 ;
        foreach ($ICs as $i=>$ic)
          { if ($j > 12) break ; $j ++ ;
            $ID = $IDs[$i] ;
            $ani = set_name ($ID) ;
    ...
    if (! $ICknown[1]) { $marked = $empty ;  CONSANG (1) ; }    // Sire
    if (! $ICknown[2]) { $marked = $empty ;  CONSANG (2) ; }    // Dam
    $COR= 2.0 * $COIs[0] / sqrt ((1. + $COIs[1]) * (1. + $COIs[2])) ;
    $COR = substr ($COR, 0, 8) ;
    if (! $COR) $COR = "n.a." ; 
    $f1 = substr ($COIs[1], 0, 8) ; $f2= substr ($COIs[2], 0, 8) ; 
    if (! $f1) $f1 = "n.a." ; if (! $f2) $f2 = "n.a." ;
    $SSDcor = (100*$COR);
    $SSDsire = (100*$f2);
    $SSDdam = (100*$f1);
}
